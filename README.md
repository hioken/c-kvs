...
- **最大メモリ使用量制限**
  - 本番想定: 約500Mbyte (524,288,000 byte)
  - テストモード: 1024byte
- **クラッシュ回避機構**
...
## 保留・非実装項目
### 保留項目（基本機能完成後、余力があれば実装を検討する項目）
- **接続・生存確認コマンド**: `PING`（PONGを返す）や `ECHO`（引数の文字列をそのまま返す）といった、パースや動作検証の基準となる基礎コマンド。
- **動的リサイズ**: ハッシュテーブルの負荷に応じたバケット数の自動拡張・再ハッシュ機構。
- **Sorted Set**: スコア付きで順序を管理するデータ型（本家RedisのZSET相当）。
...
### 非実装項目（実装コストに対するアピール価値の低さ、または難易度の高さから明確にスコープ外とする項目）
